...
@click.option('--runtime-worker', default=None, type=float,
    help="Soft time limit, in seconds, given to a worker to process one "
         "upload (default: osint_webui_runtime_worker config value, "
         "itself defaulting to 30). Past this, the adaptive concurrency "
         "stops growing; past twice this, it is reduced.")
@click.pass_obj
def upload(common, knowledge, incremental, runtime_worker):
    """Upload data to webui knowledge"""
    from tqdm import tqdm

    common.configure_logging()
    logger.debug('upload: knowledge=%s incremental=%s runtime_worker=%s log_file=%s log_level=%s',
        knowledge, incremental, runtime_worker, common.log_file, common.log_level)
...
def clean_knowledge(common, knowledge, max_workers):
    ...
    common.configure_logging()
    logger.debug('clean_knowledge: knowledge=%s max_workers=%s log_file=%s log_level=%s',
        knowledge, max_workers, common.log_file, common.log_level)
...
@click.option('--max-tokens', help="Max tokens generated by the model", default=900)
@click.option('--repeat-penalty', help="Repeat penalty of the model", default=1.15)
@click.pass_obj
def create_knowledge(common, name, description, prompt, base_model, num_ctx, max_tokens, repeat_penalty):
    """Create a knowledge and its associated model"""
    common.configure_logging()
    logger.debug('create_knowledge: name=%s base_model=%s log_file=%s log_level=%s',
        name, base_model, common.log_file, common.log_level)
...
def clean(common):
    """Clean all files !!!"""
    common.configure_logging()
    logger.debug('clean: log_file=%s log_level=%s', common.log_file, common.log_level)
...
def clean_orphans(common, assume_yes):
    ...
    common.configure_logging()
    logger.debug('clean_orphans: assume_yes=%s log_file=%s log_level=%s',
        assume_yes, common.log_file, common.log_level)
...
def stats(common, knowledge):
    """Stats"""
    common.configure_logging()
    logger.debug('stats: knowledge=%s log_file=%s log_level=%s',
        knowledge, common.log_file, common.log_level)
...
def dump(common, knowledge, output):
    """Stats"""
    common.configure_logging()
    logger.debug('dump: knowledge=%s output=%s log_file=%s log_level=%s',
        knowledge, output, common.log_file, common.log_level)
...
def add_function(common, knowledge, fname):
    ...
    common.configure_logging()
    logger.debug('add_function: knowledge=%s fname=%s log_file=%s log_level=%s',
        knowledge, fname, common.log_file, common.log_level)
